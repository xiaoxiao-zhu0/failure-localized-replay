...
class NCMClassifier(DynamicModule):
    ...
    def __init__(self, normalize: bool = True, num_classes: int | None = None):
        """
        :param normalize: whether to normalize the input with
            2-norm = 1 before computing the distance.
        :param num_classes: optional fixed output size. This is useful for
            generic evaluation experiences that do not expose a class
            timeline.
        """
        super().__init__()
        ...
        self.max_class = -1 if num_classes is None else num_classes - 1
