        ...
        for dataset in self.replay_plugin.storage_policy.buffer_datasets:
            ...
            for x, y, _ in dl:
                ...
                if label in class_means:
                    ...
                    class_means[label] = out.sum(0).cpu().detach().clone()
            # When mem_size is smaller than the number of seen classes,
            # ClassBalancedBuffer intentionally assigns zero slots to some
            # groups. Keep their existing/missing NCM prototype unchanged.
            if num_els == 0:
                continue
